...
Matrix::Matrix(const char* fileName) 
{
	...
	long int numCols,numRows;
	bool istrans;
	fread(&numCols,sizeof(long int),1,f);
	fread(&numRows,sizeof(long int),1,f);
	fread(&istrans,sizeof(bool),1,f);
	_init(NULL, numRows, numCols, istrans, true);
	this->_data=new MTYPE[numCols*numRows];
	maxLength=numCols*numRows;
	fread((this->_data),sizeof(MTYPE),numCols*numRows,f);
	fclose(f);
	this->_fileName=fileName;
}


void Matrix::copyMem(MTYPE* dst)
{
	memcpy((void*)dst, (void*)(this->_data), sizeof(MTYPE)*_numCols*_numRows);
}

#include <direct.h>
void Matrix::toFile(int epoch)
{
	FILE* f;

	if (epoch==-1)
	{
		f=fopen(this->_fileName.c_str(),"wb");
	}else{
		size_t pathPosition=this->_fileName.find_last_of('\\')+1;
		string path,filename;
		path.assign(this->_fileName.c_str(),pathPosition);
		filename.assign(&(this->_fileName[pathPosition]),this->_fileName.length()-pathPosition);

		char ffn[260];
		sprintf(ffn,"%s%d\\",path.c_str(),epoch);
		_mkdir(ffn);
		sprintf(ffn,"%s%d\\%s",path.c_str(),epoch,filename.c_str());
		f=fopen(ffn,"wb");
	}

	long int numCols,numRows;
	bool istrans=this->isTrans();
	fwrite(&this->_numCols,sizeof(long int),1,f);
	fwrite(&this->_numRows,sizeof(long int),1,f);
	fwrite(&istrans,sizeof(bool),1,f);
	fwrite((this->_data),sizeof(MTYPE),_numCols*_numRows,f);
	fclose(f);
}
...
//copy data from like
Matrix::Matrix(Matrix* like)
{
    this->_data = NULL;
    this->_trans = CblasNoTrans;
	if (like->getData() != NULL) {
		this->_updateDims(like->getNumRows(), like->getNumCols());

		this->_data = like->getData();
		this->_ownsData = false;
		this->_trans = like->isTrans()?CblasTrans:CblasNoTrans;
...
//	_init(like->_data, like->getNumRows(), like->getNumCols(), like->isTrans(), false);
// 	this->_data = new MTYPE[this->_numElements];
// 	for (long int i = 0; i < _numRows; i++) {
// 		for (long int j = 0; j < _numCols; j++) {
// 			(*this)(i,j) = (*like)(i,j);
// 		}
// 	}
	//memcpy(this->_data,like->getData(),this->_numElements*sizeof(MTYPE));
}
...
void Matrix::TransposeRightMult(const Matrix &b, MTYPE scaleAB, Matrix &target) const 
{
	Matrix& transThis=this->transpose();
	Matrix& transB=b.transpose();	
	target.resize(transB._numRows,transThis._numCols );
	target.addProduct(transB,transThis, scaleAB, 0);
	delete &transThis;
	delete &transB;
}
...
Matrix& Matrix::transpose(bool hard) const 
{
	...
	meTrans._fileName=_fileName;
    return meTrans;
}


void Matrix::transposeReVoid(bool hard, Matrix &meTrans) const 
{
	if (!hard || isTrans()) {
		return;
	}
	meTrans.resize(_numCols, _numRows);
	for (long int i = 0; i < _numRows; i++) {
		for (long int j = 0; j < _numCols; j++) {
			meTrans(j, i) = (*this)(i, j);
		}
...
void Matrix::randomizeUniform(VSLStreamStatePtr stream,MTYPE a,MTYPE b) 
{
    assert(stream != NULL);
    MKL_UNIFORM(MKL_UNIFORM_RND_METHOD, stream, getNumElements(), this->_data, a, b);
}
